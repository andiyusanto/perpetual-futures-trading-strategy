...
class ShadowTradeStore:
    """
    Async SQLite persistence for shadow (simulated) trades.
    Writes to the `shadow_trades` table in the same database file.
    """
    ...
    async def connect(self) -> None:
        ...
        await self._db.commit()
        logger.info("ShadowTradeStore connected: %s", db_path)
    ...
    async def record_open(self, rec: TradeRecord, slippage_bps: float = 0.0) -> None:
        if not self._db:
            return
        ...
        try:
            await self._db.execute(
                """INSERT INTO shadow_trades
                   (trade_id, symbol, direction, entry_price, stop_loss, take_profit,
                    amount, size_pct, grade, confidence, open_ts,
                    simulated_slippage_bps, would_have_executed, shadow_timestamp)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    rec.trade_id, rec.symbol, rec.direction,
                    rec.entry_price, rec.stop_loss, rec.take_profit,
                    rec.amount, rec.size_pct, rec.grade, rec.confidence,
                    rec.open_ts, slippage_bps, 1, now_ms,
                ),
            )
            await self._db.commit()
        except Exception as exc:
            logger.error("shadow_store_record_open_error: %s", exc)
    ...
    async def record_close(
        ...
        try:
            ...
            logger.error("shadow_store_record_close_error: %s", exc)
    ...
    async def daily_summary(self, day_start_ms: Optional[int] = None) -> dict:
        ...
        async with self._db.execute(
            """SELECT COUNT(*) as total,
                      ...
                      MIN(pnl_pct) as worst,
                      MAX(pnl_pct) as best,
                      AVG(simulated_slippage_bps) as avg_slip
               FROM shadow_trades
               WHERE open_ts >= ? AND close_ts IS NOT NULL""",
            (day_start_ms,),
        ) as cur:
        ...
        if row is None or row["total"] == 0:
            return {"trades": 0, "wins": 0, "win_rate": 0.0, "total_pnl": 0.0,
                    "worst": 0.0, "best": 0.0, "avg_slip_bps": 0.0}
        ...
        return {
            ...
            "worst":        float(row["worst"] or 0.0),
            "best":         float(row["best"] or 0.0),
            "avg_slip_bps": float(row["avg_slip"] or 0.0),
        }
    ...
    async def fetch_trades(self, limit: int = 100) -> List[dict]:
        if not self._db:
            return []
        async with self._db.execute(
            "SELECT * FROM shadow_trades ORDER BY open_ts DESC LIMIT ?", (limit,)
        ) as cur:
            rows = await cur.fetchall()
        return [dict(r) for r in rows]
